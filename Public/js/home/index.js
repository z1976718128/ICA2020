	...
	new Vue({
		...
		methods: {
			joinSubmit: function() {
				...
				if (!e.is_sub) {
					...
					// $.post("/2020/ajax/joinSubmit", t, function(t) {
		...
		methods: {
			vSubmit: function() {
				...
				if (!e.is_v_sub) {
					...
					// $.post("/2020/ajax/vSubmit", t, function(t) {
		...
		methods: {
			getSpeakers: function(t) {
				...
				// $.post("/2020/ajax/getSpeakers", n, function(t) {
				// 	if (0 == t.status) {
				// 		a.items = t.data;
		...
		methods: {
			getIndexModal: function(t) {
				...
				// $.post("/2020/ajax/getModal", n, function(t) {
				// 	if (0 == t.status) {
				// 		var n = $("#lang").val();
...
//			backgroundColor: "#fff"
	...
	var l = $("#ica2020").offset().top - 500;
	 c.addTween(l, TweenMax.to($("html"), .1, {
		css: {
			backgroundColor: "#2b91a1"
		}
	})),
	c.addTween(l, TweenMax.to($("#menu_ul li a , #ica2020 .content_title"), .2, {
		css: {
			color: "#fff"
		}
	...
	})), c.addTween(l - 300, TweenMax.to($("#ica2020 .content_title"), .2, {
		css: {
			opacity: 1,
			bottom: "0px"
		}
	})), c.addTween(l - 200, TweenMax.to($("#ica2020 .content_text"), .2, {
		css: {
			opacity: 1,
			bottom: "0px"
		}
	...
	})), c.addTween(m, TweenMax.to($("html"), .1, {
		css: {
			backgroundColor: "#2b91a1"
		}
	...
	})), c.addTween(m - 200, TweenMax.to($("#schedule .content_title"), .1, {
		css: {
			opacity: 1,
			bottom: "0px"
		}
